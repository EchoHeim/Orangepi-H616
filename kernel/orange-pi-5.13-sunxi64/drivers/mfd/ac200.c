			   ...
			   const struct i2c_device_id *id)
{
	...
	if (IS_ERR(ac200->regmap)) {
		printk("=== devm_regmap_init_i2c failed! ===\n");
	...
	{
		printk("=== devm_regmap_init_i2c success! ===\n");
	}
	...
	if (ret)
	{
		printk("=== regmap_write error! === AC200_SYS_IRQ_ENABLE \n");
		return ret;
	}
	...
	if (ret)
	{
		printk("=== regmap_add_irq_chip error! === \n");
		return ret;
	}
	...
	if (ret) {
		printk("=== devm_mfd_add_devices error! === \n");
