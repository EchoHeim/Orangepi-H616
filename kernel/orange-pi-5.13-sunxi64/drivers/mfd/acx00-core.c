...
static int __init acx00_i2c_init(void)
{
	...
	{
		ac200_used = value;		
		printk("=== sys_script_get_item err! ===\n");
	}
	else
		printk("=== sys_script_get_item success! ===\n");
	...
	if (ac200_used) {
		...
		if (emac_ephy & SUNXI_AC300_KEY) {
			...
			if (ret != 0)
				...
				printk("=== register acx00 I2C driver success! ===\n");
		}
		...
		if (ret == 2) {
			...
			if (IS_ERR_OR_NULL(vcc_ave)) {
				...
				ret = regulator_enable(vcc_ave);
				if (IS_ERR(vcc_ave)) {
					pr_err("[%s]: vcc_ave enable failed!\n", __func__);
				}
			}
			printk("=== get ave_regulator_name success! ===\n");
		} else {
			printk("=== get ave_regulator_name failed! ===\n");
			pr_err("[ac200] get ave_regulator_name failed!\n");
		}
#if defined(CONFIG_ARCH_SUN50IW9)
		printk("=== begin sys_script_get_item! ===\n");

		if (!pwm) {
			printk("=== run sys_script_get_item! ===\n");
			ret = sys_script_get_item(key_name, "tv_pwm_ch", &value, 1);
			if (ret == 1) {
				printk("=== begin pwm_request! ===\n");
				pwm = pwm_request(value, NULL);
				if (!IS_ERR_OR_NULL(pwm)) {
					pwm_config(pwm, 205, 410);
					pwm_enable(pwm);
				...
				pr_err("[ac200] Get tv_pwm_ch failed\n");
				printk("=== [ac200] Get tv_pwm_ch failed! ===\n");
			}
		}
#endif
	}
